    ...
    <section onClick={()=>{setModalAddBuku(false)}} className="container-modal-buku fixed px-3 py-8 overflow-x-hidden overflow-y-auto  top-0 left-0 z-30 w-full h-screen grid place-items-center bg-[rgba(0,0,0,0.5)]">
            <div onClick={(e)=>{e.stopPropagation()}} className="card-modal   p-4 rounded-md bg-slate-100">
                <form onSubmit={AddBuku}>
                    ...
                    <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" disabled={(loadingForm) ? true : false}>{(loadingForm)? <Loading/> : 'Kirim'}</button>
